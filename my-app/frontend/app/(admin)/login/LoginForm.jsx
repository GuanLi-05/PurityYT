...
export default function LoginForm({ setAlertShow, setAlertMessage }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!checkPassword(pw)) {
      setAlertMessage("Password contains invalid characters.");
      setAlertShow(true);
      return;
    }
    handleLogin(em, pw)
  ...
  const checkPassword = (pw) => {
    return pw != null && pw != undefined && /[\x00-\x7F]+/.test(pw);
  }
      ...
      <form className="mt-2" onSubmit={handleSubmit} noValidate>
        ...
        <div className="flex flex-col space-y-4">
          ...
          <span className="max-w-sm text-[0.8rem] text-neutral-500 dark:text-neutral-400">
            ...
            <Link href="/register" className="font-medium text-neutral-700 dark:text-neutral-300 hover:underline">
              Register
            </Link>
          </span>
        </div>
